...
/** Source Serif 4 (display) + IBM Plex Mono (kicker / meta) — a refined,
 *  journal-like pairing for the share cards. */
export async function loadOgFonts(): Promise<OgFont[]> {
  ...
  return [
    serif && { name: "Source Serif 4", data: serif, weight: 600 as const },
    mono && { name: "IBM Plex Mono", data: mono, weight: 500 as const },
  ].filter(Boolean) as OgFont[];
}

/** Fetch a same-origin image and inline it as a data URL so it embeds
