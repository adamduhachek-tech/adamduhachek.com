/**
 * Sparkmarks: deterministic miniature chart abstractions, seeded by an essay's
 * slug and shaped by its primary visualization type. They stand in for cover
 * images (the essays are self-contained D3 documents with no thumbnails) and
 * reappear on the generated OG cards.
 *
...
/** Palette slots: 0 ink, 1 maroon, 2 gold, 3 navy, 4 faint ink. */
export const SPARK_LIGHT = ["#1b2130", "#9b2335", "#b07c2e", "#16233f", "#7b8398"];
export const SPARK_DARK = ["#e7e2d6", "#d06b78", "#d9b05e", "#9fb2d4", "#8b93a6"];
...
function familyFor(primaryViz: string | null, rng: () => number): Family {
  ...
  if (v.includes("choropleth") || v.includes("map") || v.includes("heatmap")) return "grid";
  if (v.includes("slope") || v.includes("paired") || v.includes("trajector") || v.includes("waterfall")) return "slope";
  if (v.includes("stream") || v.includes("time-series") || v.includes("timeline") || v.includes("ribbon") || v.includes("line")) return "lines";
  if (v.includes("distribution") || v.includes("ridge")) return "ridges";
  if (v.includes("scatter") || v.includes("relationship") || v.includes("plane")) return "dots";
  if (v.includes("quantile") || v.includes("fan")) return "fan";
  if (v.includes("dumbbell") || v.includes("forest") || v.includes("dot-and-interval") || v.includes("beeswarm")) return "dumbbell";
  if (v.includes("multiple") || v.includes("small") || v.includes("bump")) return "multiples";
